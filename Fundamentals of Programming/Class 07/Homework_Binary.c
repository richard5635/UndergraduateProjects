...
int main(int argc, const char * argv[])
{
    ...
    while( head <= tail )
    {
        //Three initial conditions: The keyword is in the middle, below mid, or above mid
        if ( data[mid] < key )
            head = mid + 1;
            //If the middle value is smaller than key, make head equals to mid added by 1 (the search range is narrowed to the lower half of the list)
        else if ( data[mid] == key )
        {
            printf("%d found at location %d.\n", key, mid+1);
            break;
        }
        else //If the key is located above mid.
        {
