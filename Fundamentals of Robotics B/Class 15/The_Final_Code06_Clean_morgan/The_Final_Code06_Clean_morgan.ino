...
void loop()
{
  ...
  if (SensorAReading<low &&  SensorBReading<low && SensorCReading>high  &&  SensorDReading>high &&   SensorEReading<low)
  {
    // left turn 
    goLeft();
  }

  else if (SensorAReading<low &&  SensorBReading<low && SensorCReading<low &&  SensorDReading>high &&   SensorEReading>high)
  {
    // right turn
    goRight();
  }
  
  else if (SensorAReading<low &&  SensorBReading<low && SensorCReading>high &&  SensorDReading>high &&   SensorEReading>high)
  {
    // "T" cross(turn left according to left rule)
    goLeft();
  }
  else if (SensorAReading>high &&  SensorBReading<low && SensorCReading<low &&  SensorDReading>high &&   SensorEReading<low)
  {
    // straight line 
    advance();
  }
  
  else if (SensorAReading>high &&  SensorBReading<low && SensorCReading>high &&  SensorDReading>high &&   SensorEReading<low)
  {
    // left>straight ,then turn left
    goLeft();
  }
  
  else if (SensorAReading>high &&  SensorBReading<low && SensorCReading<low &&  SensorDReading>high &&   SensorEReading>high)
  {
    //straight>right, then advance
    advance();
  }
  
  else if (SensorAReading>high && SensorBReading<low &&   SensorCReading>high &&  SensorDReading>high &&   SensorEReading>high)
  {
    // "+" cross 
  ...
  else if (SensorAReading>high  &&  SensorBReading>high  &&   SensorCReading>high &&  SensorDReading>high &&   SensorEReading>high)
  {
    Stop();
  }
  
  else if (SensorAReading<low &&  SensorBReading<low && SensorCReading>high  &&  SensorDReading<low &&   SensorEReading<low)
  {
    // small left turn, this is to be used instead of the PID
    goSmallLeft();
  }
  
  else if (SensorAReading<low &&  SensorBReading<low && SensorCReading<low &&  SensorDReading<low &&   SensorEReading>high)
  {
    // small right turn, this is to be used instead of the PID
    goSmallRight();
  }
...
//The function to make the robot turn small right, used instead of PID
void goSmallRight()
{
  digitalWrite(pinLin1,150);
  analogWrite(pinLin2,LOW);
...
// The function to make the robot turn small left, used instead of PID
void goSmallLeft()
{
  digitalWrite(pinLin1,LOW);
  analogWrite(pinLin2,150);
